...
#define MODE_SELECT_PIN 1 /** Input pin for mode select switch. HIGH = Mode 0 (original), LOW = Mode 1 (auto rotate-then-scoop). */
...
#define BRIEF_ROTATE_TIME 600 /** Duration (ms) to rotate plate ~35 degrees with ramp-up. Adjust as needed. */
#define ROTATE_TO_SCOOP_DELAY 400 /** Delay (ms) between plate stop and scoop start in Mode 1. */
#define FEED_WAIT_TIME 6500 /** Time (ms) to wait for user to eat before auto-returning in Mode 1. */
...
void brief_rotate_step();  // (Mode 1) Rotates plate briefly (~35 degrees) then switches to descend_step.
...
void setup() {
  ...
  pinMode(MODE_SELECT_PIN, INPUT_PULLUP);      // Mode select: HIGH = Mode 0, LOW = Mode 1
...
void wait_mode() {
  ...
  if (digitalRead(INPUT_PIN) == LOW) {
    ...
    if (digitalRead(MODE_SELECT_PIN) == HIGH) {
      // Mode 0: Quick press = scoop, Long press = rotate while held
      timestamp = millis();
      ...
      // Mode 1: Press = brief auto-rotate then scoop
      while(digitalRead(INPUT_PIN) == LOW) {}
      switch_mode(brief_rotate_step);
    }
  }
...
/**
 * (Mode 1) Rotates the plate briefly for BRIEF_ROTATE_TIME ms (~35 degrees) with ramp-up, then switches to descend_step.
 * @see descend_step
 */
void brief_rotate_step() {
  if (pre) {
    timestamp = millis();
  }

  unsigned long elapsed = millis() - timestamp;
  if (elapsed >= BRIEF_ROTATE_TIME) {
    DCMotor::set_speed(0);
    delay(ROTATE_TO_SCOOP_DELAY);  // Settle time before scooping
    switch_mode(descend_step);
  } else {
    // Ramp up from 1/3 speed to full speed over the first second
    DCMotor::set_speed(max(DC_MOTOR_SPEED * min(elapsed, 1000) / 1000, DC_MOTOR_SPEED / 3));
  }
  check_low_power();
}
...
/**
 * Waits for user to eat the food.
 * Mode 0: Switches to return step on user input.
 * Mode 1: Auto-returns after FEED_WAIT_TIME or on user input, whichever comes first.
 * @see return_step
 */
void feed_wait_step() {
  if (pre) {
  ...
  bool timeout = (digitalRead(MODE_SELECT_PIN) == LOW) && (millis() - timestamp >= FEED_WAIT_TIME);
  if (timeout || digitalRead(INPUT_PIN) == LOW || read_joystick_button()) {
